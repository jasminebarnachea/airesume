...
function SettingsPage({user,setUser,notify}:{user:User;setUser:React.Dispatch<React.SetStateAction<User|null>>;notify:(text:string)=>void}) {
  ...
  return <div className="page-content"><div className="page-heading"><div><div className="eyebrow"><Settings size={14}/> Account preferences</div><h1>Settings</h1><p>Manage the information connected to this signed-in account.</p></div></div><form className="card panel settings-form" onSubmit={event=>{event.preventDefault();setUser(draft);if(draft.role==="Applicant"){localStorage.setItem("careerbridge_signup_user",JSON.stringify(draft));const existing=JSON.parse(localStorage.getItem("careerbridge_applicants")||"[]") as User[];localStorage.setItem("careerbridge_applicants",JSON.stringify([...new Map([...existing,draft].map(x=>[x.email.toLowerCase(),x])).values()]))}notify("Settings saved successfully")}}><div className="panel-title"><div><h2>Profile information</h2><p>Used on your account and applicant conversations.</p></div></div><div className="form-two"><label>Full name<input value={draft.name} onChange={e=>setDraft({...draft,name:e.target.value})} required/></label><label>Email address<input type="email" value={draft.email} onChange={e=>setDraft({...draft,email:e.target.value})} required/></label></div>{user.role==="Applicant"&&<label>Profile image<input type="file" accept="image/*" onChange={e=>{const file=e.target.files?.[0];if(file){const reader=new FileReader();reader.onload=()=>setDraft({...draft,avatar:String(reader.result)});reader.readAsDataURL(file)}}}/></label>}<div className="settings-toggles"><label><input type="checkbox" checked={applicationUpdates} onChange={e=>setApplicationUpdates(e.target.checked)}/><span><b>Application updates</b><small>Receive status and interview notifications.</small></span></label><label><input type="checkbox" checked={emailUpdates} onChange={e=>setEmailUpdates(e.target.checked)}/><span><b>Email notifications</b><small>Receive important recruitment messages by email.</small></span></label></div><div className="modal-actions"><button className="primary"><Check size={15}/> Save settings</button></div></form></div>
}
...
function MessagesPage({ role,messages,setMessages,user,applicants }: { role: Role;messages:SharedMessage[];setMessages:React.Dispatch<React.SetStateAction<SharedMessage[]>>;user:User;applicants:SubmittedApplication[] }) {
  ...
  return <div className="page-content"><div className="page-heading"><div><h1>Messages</h1><p>Secure conversations and hiring updates.</p></div><button className="primary"><Plus size={16}/> New message</button></div><div className="messages-layout card">
...
function JobPostingsPage({ role,notify,records,setRecords,setSubmitted,user }:{role:Role;notify:(s:string)=>void;records:JobRecord[];setRecords:React.Dispatch<React.SetStateAction<JobRecord[]>>;setSubmitted:React.Dispatch<React.SetStateAction<SubmittedApplication[]>>;user:User}) {
  ...
  const analyzeAndApply=async(e:FormEvent<HTMLFormElement>)=>{
    e.preventDefault();if(!applying)return;setMatching(true);
    const f=new FormData(e.currentTarget);const resume=f.get("resume");
    try{
      if(!(resume instanceof File))throw new Error("Please select your resume.");
      ...
      const analysis=await response.json();if(!response.ok)throw new Error(analysis.error);
      ...
      if(!routedMatches.length||!openJobs.length)throw new Error("There are no open jobs available for matching.");
      ...
      const routedApplications:SubmittedApplication[]=routedMatches.flatMap((match:{title:string;score:number;skillScore?:number;qualificationScore?:number;missingSkills?:string[]},index:number)=>{
        const job=openJobs.find(j=>j.title.toLowerCase()===String(match.title||"").toLowerCase());
        if(!job)return [];
        ...
        return [{id:baseId+index,name:String(f.get("name")),email:String(f.get("email")),applicantAvatar:user.avatar,job:job.title,office:job.office,score,skillScore,qualificationScore:Number(match.qualificationScore||score),skills:Array.isArray(analysis.skills)?analysis.skills:[],status:shortlisted?"AI shortlisted":"Under review",resumeUrl,resumeName:resume.name,resumeType:resume.type,extractedText:String(analysis.extractedText||""),summary:String(analysis.summary||""),education:Array.isArray(analysis.education)?analysis.education:[],experience:Array.isArray(analysis.experience)?analysis.experience:[],qualifications:Array.isArray(analysis.qualifications)?analysis.qualifications:[],missingSkills:Array.isArray(match.missingSkills)?match.missingSkills:analysis.missingSkills||[],interviewSuggestions:Array.isArray(analysis.interviewSuggestions)?analysis.interviewSuggestions:[]}];
      });
      ...
      notify("Resume uploaded successfully.");setApplying(null)
    }catch(err){notify(err instanceof Error?err.message:"Resume analysis failed")}finally{setMatching(false)}
  };
  const blank:JobRecord={id:0,title:"",office:"Information Technology",type:"Full-time",location:"Main Campus",description:"",requirements:"",status:"Open",applicants:0,match:0};
  return <div className="page-content recruit-page"><div className="page-heading"><div><div className="eyebrow"><BriefcaseBusiness size={14}/> Recruitment opportunities</div><h1>{role==="Applicant"?"Available school jobs":"Job postings"}</h1><p>{role==="Applicant"?"Upload one resume and Groq will match you to the most relevant open position.":"Create, edit, open, or close positions visible to applicants."}</p></div>{role==="Applicant"?<button className="primary" onClick={()=>setApplying({id:-1,title:"Resume job matching",office:"Automatic office routing",type:"AI matching",location:"All school campuses",description:"Your resume will be compared with every open school position.",requirements:"Groq will extract skills, experience, education, and qualifications before selecting the strongest job match.",status:"Open",applicants:0,match:0})}><Upload size={17}/> Upload resume</button>:<button className="dark-button" onClick={()=>setEditing(blank)}><Plus size={17}/> Create job posting</button>}</div>
    ...
    {editing&&<div className="modal-backdrop" onClick={()=>setEditing(null)}><form className="modal job-modal card" onClick={e=>e.stopPropagation()} onSubmit={save}><div className="modal-head"><div><h2>{editing.id?"Edit job posting":"Create job posting"}</h2><p>Open positions become immediately visible in the applicant portal.</p></div><button type="button" onClick={()=>setEditing(null)}><X/></button></div><div className="form-two"><label>Position title<input required value={editing.title} onChange={e=>setEditing({...editing,title:e.target.value})} placeholder="e.g. School Registrar"/></label><label>Responsible office<select value={editing.office} onChange={e=>setEditing({...editing,office:e.target.value})}><option>Information Technology</option><option>Academic Office</option><option>Human Resources</option><option>Accounting Office</option></select></label><label>Employment type<select value={editing.type} onChange={e=>setEditing({...editing,type:e.target.value})}><option>Full-time</option><option>Part-time</option><option>Contract</option><option>Temporary</option></select></label><label>Campus location<input value={editing.location} onChange={e=>setEditing({...editing,location:e.target.value})}/></label></div><label>Description<textarea required value={editing.description} onChange={e=>setEditing({...editing,description:e.target.value})} placeholder="Position responsibilities and summary"/></label><label>Qualifications and required skills<textarea required value={editing.requirements} onChange={e=>setEditing({...editing,requirements:e.target.value})} placeholder="Education, experience, skills, and certifications"/></label><label className="status-toggle">Posting status<select value={editing.status} onChange={e=>setEditing({...editing,status:e.target.value as "Open"|"Closed"})}><option>Open</option><option>Closed</option></select><small>{editing.status==="Open"?"Applicants can see and apply to this job.":"This job is hidden from applicants."}</small></label><div className="modal-actions"><button type="button" className="secondary" onClick={()=>setEditing(null)}>Cancel</button><button className="dark-button">{editing.status==="Open"?"Save and publish":"Save as closed"}</button></div></form></div>}
    {applying&&<div className="modal-backdrop" onClick={()=>!matching&&setApplying(null)}><form className="modal job-modal card resume-upload-modal" onClick={e=>e.stopPropagation()} onSubmit={analyzeAndApply}><div className="modal-head"><div><div className="eyebrow"><Sparkles size={13}/> Secure AI resume matching</div><h2>Upload your resume</h2><p>CareerBridge will compare your qualifications with every open position.</p></div><button type="button" disabled={matching} onClick={()=>setApplying(null)}><X/></button></div><div className="form-two"><label><span>Full name</span><div className="field-with-icon"><UserRound size={16}/><input name="name" required defaultValue={user.name}/></div></label><label><span>Email address</span><div className="field-with-icon"><Mail size={16}/><input name="email" type="email" required defaultValue={user.email}/></div></label></div><label className="resume-drop-field"><input name="resume" type="file" accept=".pdf,.docx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" required/><div className="resume-drop-icon"><Upload size={23}/></div><b>Select your resume</b><span>PDF or DOCX · Maximum 10 MB</span><small>Skills, education, experience, and qualifications are extracted securely.</small></label><label className="message-field"><span>Message to Admin/HR <small>Optional</small></span><textarea name="note" placeholder="Share a short introduction or any relevant information."/></label><div className="privacy-note"><ShieldCheck size={16}/><span>Your original file remains private and is shown only to authorized Admin/HR reviewers.</span></div><div className="modal-actions"><button type="button" className="secondary" disabled={matching} onClick={()=>setApplying(null)}>Cancel</button><button className="dark-button upload-analyze-button" disabled={matching}><Sparkles size={16}/>{matching?"Parsing and matching…":"Analyze and upload"}</button></div></form></div>}
  </div>;
}
